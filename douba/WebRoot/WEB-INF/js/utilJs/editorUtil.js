	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('image.*') || !f.name.toLowerCase().match(/(?:gif|jpg|png|jpeg)$/)){ // Process
																							// only
																							// Images
			...
			var getURL = $("<button/>",{
				...
				var image = $('<img/>',{src:url,}).ajaxError(function(){
		...
		init : function( options )
		{
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													if(navigator.userAgent.match(/MSIE/i)){
														var imageStr = '<img alt="1" src="'+$('#imageList').data('current')+'"/>'
		    ...
		    $(document.body).mousedown(function(event) {
		        ...
		        if (!target.parents().is('#context-menu')) { // Clicked
																		// outside
		            $('#context-menu').remove();
		        } 
		        if (!target.parents().is('#specialchar') && (target.closest('a').html()!='<i class="fa fa-asterisk"></i>')) { // Clicked
																																		// outside
		        ...
		        if (!target.parents().is('#paletteCntr') && (target.closest('a').html()!='<i class="fa fa-font"></i>')) { // Clicked
																																	// outside
