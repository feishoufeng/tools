 ...
 *            Y轴对齐方式（默认居中）
 */
function createPoshytip(id,alignX, alignY) {
	$("#" + id).poshytip({
		className : "tip-yellowsimple",
		showOn : "none",
		alignTo : "target",
		alignX : typeof (alignX) == "undefined" ? "right" : alignX,
		alignY : typeof (alignY) == "undefined" ? "center" : alignY,
		offsetX : 5
	});
}
 ...
 * @param content 提示内容
 * @param id id名称
 * @param border_color 输入框边框颜色（默认为red）
 */
function tipShow(id,content,border_color) {
	$("#" + id).poshytip("update", content);
	$("#" + id).css({
		"border" : "1px solid " + typeof (border_color) == "undefined" ? "red" : border_color
	});
	$("#" + id).poshytip("show");
}
 ...
 * 隐藏悬浮气泡
 * @param id id名称
 * @param border_color 输入框边框颜色（默认为#707070）
 */
function tipHide(id, border_color) {
	$("#" + id).css({
		"border" : "1px solid " + typeof (border_color) == "undefined" ? "#707070" : border_color
	});
	$("#" + id).poshytip("hide");
}