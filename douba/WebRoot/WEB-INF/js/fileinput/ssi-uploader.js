...
//----------------------------UPLOADFILES------------------------------------
            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                     ...
                     '<tr><td title="'+ thisS.language.name + filename +'">' + thisS.language.name + cutFileName(filename, ext, 12) + '</td></tr>' +
                     '<tr><td>' + thisS.language.size + (file.size * Math.pow(10, -6)).toFixed(2) + ' Mb</td></tr></table>';
                };
    ...
    Ssi_upload.prototype.clear = function (action) {//clean the list of all non in progress files
        switch (action) {
            ...
            case 'completed':
                ...
                break;
            default:
            	var tableNum = $("#ssi-previewBox table").length;
            	if(tableNum > 0){
            		clear();
            	}
                clearPending(this);
    ...
    var locale = {
        ...
        zh:{
            ...
            clear: '清除',
            drag: '可直接拖拽文件',
            sizeError: '$1 文件过大，限制大小为  $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 不支持的文件类型',//$1=file extension ie(exe files are not supported)
            someErrorsOccurred: '发生异常!'
        },
        gr: {
            success: 'Επιτυχία',
