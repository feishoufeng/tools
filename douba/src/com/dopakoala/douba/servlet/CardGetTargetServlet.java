 ...
 */
@WebServlet("/card/gettarget")
public class CardGetTargetServlet extends BaseServlet {
		...
		try {
			...
			if (errcode == 0) {
				...
				if (groupUserSign != null) {
					nowmiles = groupUserSign.getNowmiles();
					targetmiles = groupUserSign.getTargetmiles();
				} else {
					errmsg = "用户未设置月跑量！";
				}
			}
			...
			dataMap.put("nowmiles", nowmiles);
			dataMap.put("targetmiles", targetmiles);
